    ...
    <div class="cuerpo">
        ...
        <div class="miniaturas">
            ...
            if(isset($_POST['buscar'])){
                ...
                $sql = "SELECT * FROM guias WHERE titulo LIKE '%$search%' OR descripcion LIKE '%$search%' OR texto LIKE '%$search%' OR usuarios_nom_us LIKE '%$search%'";
                $result = mysqli_query($db,$sql);
                $queryResults = mysqli_num_rows($result);
                if ($queryResults > 0 ){
                    while($row = mysqli_fetch_assoc($result)){
                        echo "<a href='articulo.php?titulo=".$row['titulo']."&fecha=".$row['fecha']."' style=' text-decoration:none; color:white;'><div style=' 
                                height: 100%;
                                width: 100%;
